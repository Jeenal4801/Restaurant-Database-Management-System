...
class Rem(Tk):
    def __init__(self):
        ...
        #creating table
        self.listTree = ttk.Treeview(self,height=14,columns=('name','contactno'))
        ...
        self.listTree.heading("#0", text='Manager ID')
        ...
        def ser():
             try:
                ...
                cursor.execute("Select * from manager order by idno")
                pc = cursor.fetchall()
                if pc:
                    ...
                    messagebox.showinfo("Error", "No Manager...!")
             except Error:
                #print(Error)
        ...
        def check():
                    ...
                    # Customer - Button
                    self.button = Button(self, text='View Manager(s)', width=20,bg='pink', font=('Courier new', 10), command=ser).place(x=200,y=250)
