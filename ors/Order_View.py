...
class MainWin(Tk):
    def __init__(self):
        ...
        #creating table
        self.listTree = ttk.Treeview(self,height=14,columns=('itemid','managerid','waiterid','quantity','price'))
        ...
        self.listTree.heading("#0", text='custid')
        self.listTree.column("#0", width=50,minwidth=50,anchor='center')
        self.listTree.heading("itemid", text='itemid')
        self.listTree.column("itemid", width=200, minwidth=200,anchor='center')
        self.listTree.heading("managerid", text='managerid')
        self.listTree.column("managerid", width=200, minwidth=200,anchor='center')
        self.listTree.heading("waiterid", text='waiterId')
        self.listTree.column("waiterid", width=125, minwidth=125,anchor='center')
        ...
        self.listTree.place(x=150,y=325)
        ...
        def ser():
             try:
                ...
                cursor.execute("SELECT 	customerId , itemId ,managerId  , \
                                waiterId  ,  quantity ,  price \
                                FROM ors.order   \
                                order by customerId , itemId , managerId , waiterId ;")
        ...
        def check():
                    ...
                    #self.label6 = Label(self, text="ITEM INFORMATION DETAILS",bg="blue",  font=('Courier new', 15, 'underline', 'bold'))
                    #self.label6.place(x=150, y=200)
                    

                    # Customer - Button
                    self.button = Button(self, text='View Order(s)', width=20,bg='pink', font=('Courier new', 10), command=ser).place(x=200,y=250)
